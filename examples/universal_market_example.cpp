...
struct MockBackend : public MarketBackend {
  ...
  std::string create_order(const Order &o) const override {
    std::cout << "[" << name_str << "] Order created for " << o.market.ticker
              << std::endl;
    return "mock_id";
  }
};

// Use the globally defined LiveExchange

int main() {
  MockBackend poly("Polymarket");
  MockBackend kalshi("Kalshi");

  LiveExchange.register_backend(&poly);
  LiveExchange.register_backend(&kalshi);

  // 1. Register super-ticker "BTC" across multiple exchanges
  std::cout
      << "[INFO] Registering 'BTC' super-ticker on Polymarket and Kalshi..."
      << std::endl;
  ...
  if (condition.condition.eval()) {
    std::cout << "[SUCCESS] Condition Met: BTC is available under 0.50 on at "
                 "least one registered exchange."
              << std::endl;
  } else {
  ...
  if (condition.condition.eval()) {
    std::cout << "[FAILURE] Condition still met but should not be."
              << std::endl;
  } else {
    std::cout << "[SUCCESS] Condition correctly NOT met when all exchange "
                 "prices are high."
              << std::endl;
  }
