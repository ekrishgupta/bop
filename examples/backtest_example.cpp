...
int main() {
  ...
  std::cout << "[EXAMPLE] Setting up strategy: Buy AAPL if price < 0.50"
            << std::endl;

  auto cond_order = When(Market("AAPL", *backend).Price(YES) < 0.50_usd) >>
                    (Buy(100) / Market("AAPL", *backend) / YES);
  ...
  // 5. Check results
  int64_t pos = engine.get_position(MarketId("AAPL")).raw;
  std::cout << "[EXAMPLE] Final Position AAPL (from CSV): " << pos << std::endl;

  if (pos == 100) {
    std::cout << "[EXAMPLE] SUCCESS: CSV Order was filled correctly."
              << std::endl;
  } else {
  ...
  std::string json_file = "backtest_data.json";
  {
    ...
    file << "[\n";
    file << "  {\"timestamp\": 1700000000, \"ticker\": \"TSLA\", "
            "\"yes_price\": 0.80, \"no_price\": 0.20},\n";
    file << "  {\"timestamp\": 1700000001, \"ticker\": \"TSLA\", "
            "\"yes_price\": 0.65, \"no_price\": 0.35},\n";
    file << "  {\"timestamp\": 1700000002, \"ticker\": \"TSLA\", "
            "\"yes_price\": 0.65, \"no_price\": 0.35}\n";
    file << "]\n";
  }

  // Setup TSLA strategy: Buy if TSLA < 0.75
  auto tsla_order = When(Market("TSLA", *backend).Price(YES) < 0.75_usd) >>
                    (Buy(50) / Market("TSLA", *backend) / YES);
  tsla_order.order.backend = backend.get();
  tsla_order >> engine;

  engine.run_from_json(json_file);

  int64_t tsla_pos = engine.get_position(MarketId("TSLA")).raw;
  std::cout << "[EXAMPLE] Final Position TSLA (from JSON): " << tsla_pos
            << std::endl;

  engine.report();

  if (tsla_pos == 50) {
    std::cout << "[EXAMPLE] SUCCESS: JSON Order was filled correctly."
              << std::endl;
  } else {
