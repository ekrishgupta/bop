...
public:
  /**
   * @brief Calculate Greeks using a simplified Black-Scholes for binary
   * options.
   * @param p Current market price (0.0 to 1.0)
   * @param sigma Annualized volatility
   * @param time_to_expiry Time in years
   */
  static Greeks calculate_greeks(Price price, double sigma,
    ...
    auto inv_normal_cdf = [](double p) {
      ...
      return std::sqrt(2.0) *
             std::erf(2.0 * p - 1.0); // Not exact, but illustrative
    };
...
struct Peg {
  ...
  explicit Peg(ReferencePrice r, Price o)
      : ref(r), offset(o), use_ticks(false) {}
  explicit Peg(ReferencePrice r, Ticks t)
      : ref(r), offset(Price(t.raw)), use_ticks(true) {}
};

inline Peg operator+(ReferencePrice r, Ticks t) { return Peg(r, t); }
inline Peg operator-(ReferencePrice r, Ticks t) {
  return Peg(r, Ticks(-t.raw));
}
inline Peg operator+(ReferencePrice r, Price p) { return Peg(r, p); }
inline Peg operator-(ReferencePrice r, Price p) { return Peg(r, -p); }
