...
void Database::log_order(const std::string &id, const Order &o) {
  ...
  if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
      std::cerr << "[DB] Failed to log order: " << sqlite3_errmsg(db_)
                << std::endl;
    }
...
void Database::log_fill(const std::string &id, int qty, Price price) {
  ...
  sqlite3_stmt *stmt;
  const char *sql = "INSERT INTO fills (order_id, qty, price, timestamp_ms) "
                    "VALUES (?, ?, ?, ?);";
  ...
  if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
      std::cerr << "[DB] Failed to log fill: " << sqlite3_errmsg(db_)
                << std::endl;
    }
    sqlite3_finalize(stmt);
  }
...
void Database::log_status(const std::string &id, OrderStatus status) {
  ...
  sqlite3_stmt *stmt;
  const char *sql = "INSERT INTO status_updates (order_id, status, "
                    "timestamp_ms) VALUES (?, ?, ?);";
  ...
  if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
      std::cerr << "[DB] Failed to log status: " << sqlite3_errmsg(db_)
                << std::endl;
    }
    sqlite3_finalize(stmt);
  }
...
void Database::log_pnl_snapshot(Price balance, Price pnl,
                                int64_t daily_pnl_raw) {
  if (!db_)
    return;
  std::lock_guard<std::mutex> lock(mtx_);
  ...
  sqlite3_stmt *stmt;
  const char *sql = "INSERT INTO pnl_history (balance, pnl, daily_pnl_raw, "
                    "timestamp_ms) VALUES (?, ?, ?, ?);";
  ...
  if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
      std::cerr << "[DB] Failed to log PnL: " << sqlite3_errmsg(db_)
                << std::endl;
    }
    sqlite3_finalize(stmt);
  }
