...
private:
  void render_dashboard() {
    ...
    std::cout
        << "================================================================\n";
    std::cout << "   BOP TRADING ENGINE DASHBOARD | Status: "
    ...
    std::cout << std::left << std::setw(20) << "Daily PnL:" << Price(daily_pnl)
    ...
    pg = const_cast<GreekEngine &>(greek_engine)
             .calculate_portfolio_greeks(get_all_positions(), backends_,
                                         volatilities);

    std::cout << "Greeks: Delta=" << std::fixed << std::setprecision(2)
              << pg.total_delta << " | Gamma=" << pg.total_gamma
              << " | Theta=" << pg.total_theta << " | Vega=" << pg.total_vega
              << "\n";
    ...
    if (last_tick > 0) {
      ...
      std::cout << std::left << std::setw(20) << "Engine Latency:" << std::fixed
                << std::setprecision(2) << latency_ms << " ms\n";
    }

    std::cout
        << "----------------------------------------------------------------\n";
    std::cout << " ACTIVE ALGORITHMS & STRATEGIES ("
              << GlobalAlgoManager.active_count() << ")\n";
    std::cout
        << "----------------------------------------------------------------\n";
