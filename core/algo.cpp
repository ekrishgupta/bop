...
bool TrailingStopAlgo::tick_impl(ExecutionEngine &engine) {
  ...
  if (price_improved) {
    best_price = current_price;
    std::cout << "[ALGO] Trailing Stop Updated Best Price: " << best_price
              << " (Current: " << current_price << ")" << std::endl;
  }
  ...
  if (now_ns - last_log_time_ns > 10e9) {
    std::cout << "[ALGO] Trailing Stop [" << parent_order.market.ticker
              << "] Current: " << current_price << " Best: " << best_price
              << " Stop: " << stop_price << std::endl;
    last_log_time_ns = now_ns;
  }
  ...
  if (triggered) {
    std::cout << "[ALGO] Trailing Stop Triggered! Market: "
              << parent_order.market.ticker << " at " << current_price
              << " (Best: " << best_price << ", Stop: " << stop_price << ")"
              << std::endl;
  ...
  if (target_price != last_quoted_price) {
    ...
    if (slice.backend) {
      std::cout << "[ALGO] Pegging " << parent_order.market.ticker << " to "
                << target_price << " (Offset: " << offset << ")" << std::endl;
  ...
  if (to_fill > 0) {
    ...
    if (slice.backend) {
      std::cout << "[ALGO] VWAP Slice: " << to_fill
                << " shares (Market Delta: " << volume_delta
  ...
  if (!bid_id.empty() || !ask_id.empty()) {
    ...
    for (const auto &r : records) {
      if (r.id == bid_id && r.status == OrderStatus::Filled) {
        std::cout << "[ALGO] MarketMaker: Bid side filled. Cancelling ask."
                  << std::endl;
        if (!ask_id.empty() && parent_order.backend)
          parent_order.backend->cancel_order(ask_id);
        bid_id = "";
        ask_id = "";
        return true;
      }
      if (r.id == ask_id && r.status == OrderStatus::Filled) {
        std::cout << "[ALGO] MarketMaker: Ask side filled. Cancelling bid."
                  << std::endl;
        if (!bid_id.empty() && parent_order.backend)
          parent_order.backend->cancel_order(bid_id);
        bid_id = "";
        ask_id = "";
        return true;
      }
...
// SORAlgo Implementation
SORAlgo::SORAlgo(const Order &o) {
  parent_order = o;
  auto data = std::get<SORData>(o.algo_params);
  b1 = data.b1;
  b2 = data.b2;
  total_qty = o.quantity;
}

bool SORAlgo::tick_impl(ExecutionEngine &engine) {
  if (!active || !b1 || !b2)
    return true;
  ...
  std::cout << "[ALGO] SOR Routing: " << b1->name() << " @ " << p1 << ", "
            << b2->name() << " @ " << p2 << std::endl;
  ...
  if (parent_order.is_buy) {
    if (p1 < p2) {
      ...
      // Same price, split 50/50 for now (liquidity-based split would need book
      // depth)
      qty_b1 = total_qty / 2;
  ...
  if (qty_b1 > 0) {
    ...
    b1->create_order(o1);
    std::cout << "[ALGO] SOR: Routed " << qty_b1 << " to " << b1->name()
              << std::endl;
  }
  ...
  if (qty_b2 > 0) {
    ...
    b2->create_order(o2);
    std::cout << "[ALGO] SOR: Routed " << qty_b2 << " to " << b2->name()
              << std::endl;
  }
