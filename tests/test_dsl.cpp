...
void test_modifiers() {
  ...
  auto o =
      Buy(100_shares) / "BTC"_mkt / YES + LimitPrice(0.65_usd) | IOC | PostOnly;
  ...
  auto o2 = Sell(200_shares) / "ETH"_mkt / NO + MarketPrice() | FOK |
            Iceberg(50_shares);
  TEST_ASSERT(o2.price.raw == 0, "Market price raw should be 0");
  TEST_ASSERT(o2.tif == TimeInForce::FOK, "TIF should be FOK");
  TEST_ASSERT(o2.display_qty.raw == 50, "Iceberg display quantity mismatch");
}
...
void test_brackets() {
  ...
  TEST_ASSERT(o.tp_price.raw == Price::from_usd(0.75).raw,
              "TakeProfit mismatch");
...
void test_composition() {
  ...
  auto cond =
      (Market("BTC").Price(YES) > 0.60_usd) && (Position("BTC"_mkt) < 100);

  // Check if we can build a conditional order with it
  auto co = When(cond) >> (Buy(10_shares) / "BTC" / YES);

  TEST_ASSERT(co.order.quantity.raw == 10,
              "Conditional order quantity mismatch");
}
